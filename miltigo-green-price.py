...
htm_string2 = 'now is' + str(time.strftime("%d-%m-%Y")) + '<br />'
...
for elm in adrs:
    ...
    print(elm)
    htm_string2 = htm_string2 + elm + '<br />'
    for el in alel:
        if el.find('div', class_='price green'):
            htm_string2 = htm_string2 + el.find('div', class_='price').contents[0] + '\t' +\
                  el.find('div', class_='price').contents[1].text + '\t' +\
                  el.find('div', class_='name').text + '\t' + \
                  el.find('div', class_='address').text + '<br />'
